...
export  function ControlsButtons(props: ControlsButtonsProps) {
  return (
            <Box className="p-3 bg-black-06 rounded-[12px] border border-black-12 gap-4">
    
            <Button
              variant="default"
              onClick={() => props.apiControls?.scrollPrev()}
              className=" p-1.5 bg-black-10 cursor-pointer border border-black-12 flex items-center justify-center transition-colors duration-200"
            >
            ...
            {Array.from({ length: Math.ceil(props.length / 4) }).map((_, index) => (
              <div
                ...
                className={`w-5 h-1 transition-colors duration-200 ${
                  Math.floor((props.current - 1) / 4) === index ? "bg-red-500" : "bg-gray-600"
                }`}
              />
            ))}
          </div>
            <Button
              ...
              className=" p-1.5 bg-black-10 cursor-pointer border border-black-12 flex items-center justify-center transition-colors duration-200"
            >
              <ArrowRight className="w-6 h-6 stroke-absolute-white" />
            </Button>
        </Box>
  )
}
